...
public class ScoreShowPnl extends JPanel{
	/** 标题面板 */
	...
	/** 按科目升序 */
	private static final String SORT_ASC_BY_SCORE_NAME = "按分数升序";
	/** 按科目降序 */
	private static final String SORT_DESC_BY_SCORE_NAME = "按分数降序";

	public ScoreShowPnl(StudentBean bean) {
		this.bean = bean;
		initUI();
		initPanel();
		configureCompents();
	...
	private void configureCboSort() {
		String[] sorts = {"按ID升序", "按ID降序", "按科目升序", "按科目降序", "按分数升序", "按分数降序"};
	...
	private void showMenuItemOnClickListener(ActionEvent e) {
		...
		if (index != -1) {
			...
			if (id > 0) {
				...
				PaperScoreBean paperScoreBean = paperScoreService.loadById(id);
				PaperBean paperBean = paperService.loadById(paperScoreBean.getPaperID());
				Long studentID = paperScoreBean.getStudentID();
				PaperResultFrm paperResultFrm = new PaperResultFrm(paperBean, studentID);
				paperResultFrm.showFrm();
			}
		}
	}
	...
	private void showListData() {
		PaperScoreService paperScoreService = new PaperScoreServiceImpl();
		List<PaperScoreBean> list = null;
		
		String search = txtSearch.getText().trim().toString();
		if (cboSortIsSelected().equals(SORT_ASC_BY_ID)){
			list = paperScoreService.listByIDAsc(search, bean.getID());
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_ID)) {
			list = paperScoreService.listByIDDesc(search, bean.getID());
		} else if (cboSortIsSelected().equals(SORT_ASC_BY_SUBJECT_NAME)) {
			list = paperScoreService.listByPaperNameAsc(search, bean.getID());
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_SUBJECT_NAME)) {
			list = paperScoreService.listByPaperNameDesc(search, bean.getID());
		} else if (cboSortIsSelected().equals(SORT_ASC_BY_SCORE_NAME)) {
			list = paperScoreService.listByScoreAsc(search, bean.getID());
		}  else if (cboSortIsSelected().equals(SORT_DESC_BY_SCORE_NAME)) {
			list = paperScoreService.listByScoreDesc(search, bean.getID());
		} else {
			list = paperScoreService.listBySearch(search, bean.getID());
		}
		
		ATableModel<PaperScoreBean> tableModel = null;
		
		tableModel = new ATableModel<PaperScoreBean>(list, 8) {

			@Override
			public Object getPropValue(PaperScoreBean bean, int columnIndex) {
				if (columnIndex == 0) {
					return bean.getScoreID();
					...
					return statusChange(bean.getStatus());
				}
				return null;	
			}
	...
	private String cboSortIsSelected() {
		if (cboSort.getSelectedIndex() == 0) {
			...
			return SORT_ASC_BY_SCORE_NAME;
		} else if (cboSort.getSelectedIndex() == 5){
			return SORT_DESC_BY_SCORE_NAME;
		} else {
			return null;
		}
	}
	...
	/**
	 * <p>Description: 状态字符串转换</p>
	 * <p>Title: statusChange</p>
	 * @param status
	 * @return
	 */
	private String statusChange(Integer status) {
		if (status == 0) {
			return "未考";
		} else if (status == 1) {
			return "已考";
