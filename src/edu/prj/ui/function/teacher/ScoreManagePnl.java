...
import edu.prj.ui.crud.paperItem.PaperResultFrm;

@SuppressWarnings("serial")
public class ScoreManagePnl extends JPanel{
	/** 标题面板 */
...
//		refreshData();
	...
	private void showListData() {
		...
		if (cboSortIsSelected().equals(SORT_ASC_BY_ID)){
			list = paperScoreService.listByIDAsc(search);
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_ID)) {
			list = paperScoreService.listByIDDesc(search);
		} else if (cboSortIsSelected().equals(SORT_ASC_BY_SUBJECT_NAME)) {
			list = paperScoreService.listByPaperNameAsc(search);
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_SUBJECT_NAME)) {
			list = paperScoreService.listByPaperNameDesc(search);
		} else if (cboSortIsSelected().equals(SORT_ASC_BY_SCORE_NAME)) {
			list = paperScoreService.listByScoreAsc(search);
		}  else if (cboSortIsSelected().equals(SORT_DESC_BY_SCORE_NAME)) {
			list = paperScoreService.listByScoreDesc(search);
		} else {
			list = paperScoreService.listBySearch(search);
		}
