...
import edu.prj.ui.crud.manager.*;

@SuppressWarnings("serial")
public class ManagerManagePnl extends JPanel{
	/** 标题面板 */
	...
	private void deleteMenuItemOnClickListener(ActionEvent e) {
		...
		if (index != -1) {
			...
			if (buttonValue == JOptionPane.YES_OPTION) {
				...
				// 获得删除结果
				Long result = managerService.delete(id);
				if (result > 0) {
	...
	private void showMenuItemOnClickListener(ActionEvent e) {
		...
		if (index != -1) {
			...
			if (id > 0) {
				// 根据id找到对象
				ManagerService managerService = new ManagerServiceImpl();
				ManagerBean bean = managerService.loadById(id);
				ManagerShowInfoFrm managerShowInfoFrm = new ManagerShowInfoFrm(bean);
				managerShowInfoFrm.showFrm();
			}
		}
	}
	...
	private void btnAddOnClickListener(ActionEvent e) {
		ManagerCreateFrm managerCreateFrm = new ManagerCreateFrm();
		managerCreateFrm.managerManagePnl = this;
		managerCreateFrm.showFrm();
		refreshData();
	}
	...
	private void showListData() {
		...
		if (cboSortIsSelected().equals(SORT_ASC_BY_ID)){
			list = managerService.listByIDAsc(search);
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_ID)) {
			list = managerService.listByIDDesc(search);
		} else if (cboSortIsSelected().equals(SORT_ASC_BY_NICKNAME)) {
			list = managerService.listByNicknameAsc(search);
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_NICKNAME)) {
			list = managerService.listByNicknameDesc(search);
		} else {
			list = managerService.listBySearch(search);
		}
