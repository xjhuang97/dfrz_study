...
import edu.prj.ui.crud.classroom.*;
import edu.prj.ui.crud.student.*;

@SuppressWarnings("serial")
public class ClassroomManagePnl extends JPanel{
	/** 标题面板 */
	...
	/** 按昵称升序 */
	private static final String SORT_ASC_BY_CLASSROOM_NAME = "按名称升序";
	/** 按昵称降序 */
	private static final String SORT_DESC_BY_CLASSROOM_NAME = "按名称降序";

	public ClassroomManagePnl() {
		initUI();
		initPanel();
		configureCompents();
		installCompents();
	...
	private void configureTblObjMenu() {
		...
		JMenuItem showMenuItem = new JMenuItem("查看学生");
	...
	private void deleteMenuItemOnClickListener(ActionEvent e) {
		...
		if (index != -1) {
			...
			if (buttonValue == JOptionPane.YES_OPTION) {
				...
				// 获得删除结果
				Long result = classroomService.delete(id);
				if (result > 0) {
	...
	private void showMenuItemOnClickListener(ActionEvent e) {
		...
		if (index != -1) {
			...
			Object obj = model.getValueAt(index, 0);
			Long id = Long.valueOf(obj+"");
			ClassroomBean classroomBean = classroomService.loadById(id);
			Long gradeID = classroomBean.getGradeID();
			Long classroomID = roomNameChange(classroomBean.getRoomName());		
			if (id > 0) {
				// 根据id找到对象
				StudentListFrm studentListFrm = new StudentListFrm(gradeID, classroomID);
				
				studentListFrm.showFrm();
			}
		}
	}
	...
	private void btnAddOnClickListener(ActionEvent e) {
		ClassroomCreateFrm classroomCreateFrm = new ClassroomCreateFrm(bean);
		classroomCreateFrm.classroomManagePnl = this;
		classroomCreateFrm.showFrm();
		refreshData();
	}
	...
	private void showListData() {
		ClassroomService gradeService = new ClassroomServiceImpl();
		...
		} else if (cboSortIsSelected().equals(SORT_ASC_BY_CLASSROOM_NAME)) {
			list = gradeService.listByClassroomNameAsc(search);
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_CLASSROOM_NAME)) {
			list = gradeService.listByClassroomNameDesc(search);
		} else {
	...
	private String cboSortIsSelected() {
		if (cboSort.getSelectedIndex() == 0) {
			...
			return SORT_ASC_BY_CLASSROOM_NAME;
		} else if (cboSort.getSelectedIndex() == 3) {
			return SORT_DESC_BY_CLASSROOM_NAME;
		} else {
			return null;
		}
	}
	...
	/**
	 * <p>Description: 班级数转换成对应的操作值</p>
	 * <p>Title: roomNameChange</p>
	 * @param classroom
	 * @return
	 */
	private Long roomNameChange(String classroom) {
		if (classroom.equals("一班")) {
			return 1L;
		} else if (classroom.equals("二班")) {
			return 2L;
		} else if (classroom.equals("三班")) {
			return 3L;
		} else {
			return 0L;
		}
