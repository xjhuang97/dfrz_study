...
import edu.prj.ui.crud.student.*;

@SuppressWarnings("serial")
public class StudentManagePnl extends JPanel{
	/** 标题面板 */
	...
	/** 按昵称升序 */
	private static final String SORT_ASC_BY_NICKNAME = "按昵称升序";
	/** 按昵称降序 */
	private static final String SORT_DESC_BY_NICKNAME = "按昵称降序";
	/** 当前学生列表窗口 */
	public StudentListFrm studentListFrm = null;
	/** 当前学生列表窗口选择的年级 */
	private Long grade = 0L;
	/** 当前学生列表窗口选择的班级 */
	private Long classroom = 0L;

	public StudentManagePnl() {
		initUI();
		initPanel();
		configureCompents();
		installCompents();
	...
	private void configureCboSort() {
		String[] sorts = {"按ID升序", "按ID降序", "按昵称升序", "按昵称降序"};
	...
	private void configureTblObj() {
		// 设置表头宽高、字体大小
		tblObj.getTableHeader().setPreferredSize(new Dimension(this.getWidth(), 35));
	...
	private void deleteMenuItemOnClickListener(ActionEvent e) {
		...
		if (index != -1) {
			...
			if (buttonValue == JOptionPane.YES_OPTION) {
				...
				// 获得删除结果
				Long result = studentService.delete(id);
				if (result > 0) {
	...
	/**
	 * <p>Description: 表格右键菜单修改选项</p>
	 * <p>Title: updateMenuItemOnClickListener</p>
	 * @param e
	 */
	private void updateMenuItemOnClickListener(ActionEvent e) {
		...
		if (index != -1) {
			...
			if (id > 0) {
				// 根据id找到对象
				StudentService studentService = new StudentServiceImpl();
				StudentBean bean = studentService.loadById(id);
				StudentUpdateFrm studentUpdateFrm = new StudentUpdateFrm(bean);
				studentUpdateFrm.studentManagePnl = this;
				studentUpdateFrm.showFrm();
			}
		}
	}
	...
	private void btnAddOnClickListener(ActionEvent e) {
		StudentCreateFrm studentCreateFrm = new StudentCreateFrm();
		studentCreateFrm.studentManagePnl = this;
		studentCreateFrm.showFrm();
		if (studentListFrm == null) {
			refreshData();
		} else {
			refreshData(grade, classroom);
	 ...
	 * @param classroom
	 */
	public void refreshData(Long grade, Long classroom) {
		this.grade = grade;
		this.classroom = classroom;
		showListData(grade, classroom);
	}
	...
	private void showListData() {
		...
		if (cboSortIsSelected().equals(SORT_ASC_BY_ID)){
			list = studentService.listByIDAsc(search);
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_ID)) {
			list = studentService.listByIDDesc(search);
		} else if (cboSortIsSelected().equals(SORT_ASC_BY_NICKNAME)) {
			list = studentService.listByNicknameAsc(search);
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_NICKNAME)) {
			list = studentService.listByNicknameDesc(search);
		} else {
			list = studentService.listBySearch(search);
		}
	
		ATableModel<StudentBean> tableModel = null;
		
		tableModel = new ATableModel<StudentBean>(list, 7) {
			...
			public Object getPropValue(StudentBean bean, int columnIndex) {
				if (columnIndex == 0) {
					...
					return hidePass(bean.getLoginPass());
				} else if (columnIndex == 3) {
					return bean.getNickname();
				} else if (columnIndex == 4) {					
					return bean.getGradeName();
				} else if (columnIndex == 5) {					
					return bean.getClassRoomName();
				} else if (columnIndex == 6) {
					return isDisabled(bean.getIsDisabled());
				}
				return null;
			}
	 ...
	 * @param classroomID
	 */
	private void showListData(Long gradeID, Long classroomID) {
		List<StudentBean> list = null;
		StudentService studentService = new StudentServiceImpl();
		
		String search = txtSearch.getText().trim().toString();
		if (cboSortIsSelected().equals(SORT_ASC_BY_ID)){
			list = studentService.listByIDAsc(search, gradeID, classroomID);
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_ID)) {
			list = studentService.listByIDDesc(search, gradeID, classroomID);
		} else if (cboSortIsSelected().equals(SORT_ASC_BY_NICKNAME)) {
			list = studentService.listByNicknameAsc(search, gradeID, classroomID);
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_NICKNAME)) {
			list = studentService.listByNicknameDesc(search, gradeID, classroomID);
		} else {
			list = studentService.listBySearch(search);
		}
	...
	/**
	 * <p>Description: 将密码设置为不可见</p>
	 * <p>Title: passwordStar</p>
	 * @param loginPass
	 * @return
	 */
	private String hidePass(String loginPass) {
	...
	private String cboSortIsSelected() {
		if (cboSort.getSelectedIndex() == 0) {
			...
			return SORT_ASC_BY_NICKNAME;
		} else if (cboSort.getSelectedIndex() == 3) {
			return SORT_DESC_BY_NICKNAME;
		} else {
			return null;
		}
	}
