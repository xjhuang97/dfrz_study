...
import edu.prj.ui.crud.teacher.*;

@SuppressWarnings("serial")
public class TeacherManagePnl extends JPanel{
	/** 标题面板 */
	...
	private void deleteMenuItemOnClickListener(ActionEvent e) {
		...
		if (index != -1) {
			...
			if (buttonValue == JOptionPane.YES_OPTION) {
				...
				// 获得删除结果
				Long result = teacherService.delete(id);
				if (result > 0) {
	...
	private void updateMenuItemOnClickListener(ActionEvent e) {
		...
		if (index != -1) {
			...
			if (id > 0) {
				// 根据id找到对象
				TeacherService teacherService = new TeacherServiceImpl();
				TeacherBean bean = teacherService.loadById(id);
				TeacherUpdateFrm teacherUpdateFrm = new TeacherUpdateFrm(bean);
				teacherUpdateFrm.teacherManagePnl = this;
				teacherUpdateFrm.showFrm();
			}
		}
	}
	...
	private void btnAddOnClickListener(ActionEvent e) {
		TeacherCreateFrm teacherCreateFrm = new TeacherCreateFrm();
		teacherCreateFrm.teacherManagePnl = this;
		teacherCreateFrm.showFrm();
		refreshData();
	}
	...
	private void showListData() {
		...
		if (cboSortIsSelected().equals(SORT_ASC_BY_ID)){
			list = teacherService.listByIDAsc(search);
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_ID)) {
			list = teacherService.listByIDDesc(search);
		} else if (cboSortIsSelected().equals(SORT_ASC_BY_NICKNAME)) {
			list = teacherService.listByNicknameAsc(search);
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_NICKNAME)) {
			list = teacherService.listByNicknameDesc(search);
		} else {
			list = teacherService.listBySearch(search);
		}
