...
import edu.prj.ui.crud.grade.*;

@SuppressWarnings("serial")
public class GradeManagePnl extends JPanel{
	/** 标题面板 */
	...
	/** 按昵称升序 */
	private static final String SORT_ASC_BY_GRADE_NAME = "按名称升序";
	/** 按昵称降序 */
	private static final String SORT_DESC_BY_GRADE_NAME = "按名称降序";

	public GradeManagePnl() {
		initUI();
		initPanel();
		configureCompents();
		installCompents();
	...
	private void deleteMenuItemOnClickListener(ActionEvent e) {
		...
		if (index != -1) {
			...
			if (buttonValue == JOptionPane.YES_OPTION) {
				...
				// 获得删除结果
				Long result = gradeService.delete(id);
				if (result > 0) {
	...
	private void btnAddOnClickListener(ActionEvent e) {
		GradeCreateFrm gradeCreateFrm = new GradeCreateFrm(bean);
		gradeCreateFrm.gradeManagePnl = this;
		gradeCreateFrm.showFrm();
		refreshData();
	}
	...
	private void showListData() {
		...
		if (cboSortIsSelected().equals(SORT_ASC_BY_ID)){
			list = gradeService.listByIDAsc(search);
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_ID)) {
			list = gradeService.listByIDDesc(search);
		} else if (cboSortIsSelected().equals(SORT_ASC_BY_GRADE_NAME)) {
			list = gradeService.listByGradeNameAsc(search);
		} else if (cboSortIsSelected().equals(SORT_DESC_BY_GRADE_NAME)) {
			list = gradeService.listByGradeNameDesc(search);
		} else {
			list = gradeService.listBySearch(search);
		}
		...
		tableModel = new ATableModel<GradeBean>(list, 6) {
			...
			public Object getPropValue(GradeBean bean, int columnIndex) {
				if (columnIndex == 0) {
					...
					String createOn = bean.getCreateOn();
					return createOn.substring(0, createOn.length()-2);
				} else if (columnIndex == 3) {
					return bean.getCreateBy();
				} else if (columnIndex == 4) {
					String updateOn = bean.getUpdateOn();
					return updateOn.substring(0, updateOn.length()-2);
		...
		Integer widthDate = 200;
		// 设置创建日期列表宽度
		tblObj.getColumnModel().getColumn(2).setPreferredWidth(widthDate);   
		tblObj.getColumnModel().getColumn(2).setWidth(widthDate);   
		tblObj.getColumnModel().getColumn(2).setMaxWidth(widthDate);
		tblObj.getColumnModel().getColumn(2).setMinWidth(widthDate);
		
		// 设置创建日期列表宽度
		tblObj.getColumnModel().getColumn(4).setPreferredWidth(widthDate);   
		tblObj.getColumnModel().getColumn(4).setWidth(widthDate);   
		tblObj.getColumnModel().getColumn(4).setMaxWidth(widthDate);
		tblObj.getColumnModel().getColumn(4).setMinWidth(widthDate);
	}
	...
	private String cboSortIsSelected() {
		if (cboSort.getSelectedIndex() == 0) {
			...
			return SORT_ASC_BY_GRADE_NAME;
		} else if (cboSort.getSelectedIndex() == 3) {
			return SORT_DESC_BY_GRADE_NAME;
		} else {
			return null;
		}
	}
