...
public class PaperCreateFrm extends JFrame{
	/** 内容面板 */
	...
	/** 年、月、日下拉框尺寸  */
	private Dimension cboDateDimension = new Dimension(130, 40);
	/** 按钮尺寸 */
	...
	public PaperCreateFrm(UserBean bean) {
		this.bean = bean;
		initUI();
		initFrame();
		configureCompents();
	...
	private void configureCboSubject() {
		String[] subjects = {"请选择科目", "语文", "数学", "英语"};
		cboSubject = new JComboBox(subjects);
	...
	private void btnSubmitOnClickListener(ActionEvent e) {
		...
		if (paperName == null || paperName.isEmpty()) {
			...
			paperBean.setSubjectID(Long.valueOf(cboSubject.getSelectedIndex()));
			paperBean.setPaperName(paperName);
			paperBean.setTotalScore(totalScore);
			paperBean.setPerScore(perScore);
			paperBean.setQuestionNum(questionNum);
			paperBean.setExamMinute(examMinute);
			...
			paperBean.setEndOn(endOn);
			
			if (paperService.insert(paperBean) > 0) {
	...
	private void refresh() {
		...
		cboStartOnYear.setSelectedIndex(0);
		cboStartOnMonth.setSelectedIndex(0);
		cboStartOnDay.setSelectedIndex(0);
		cboEndOnYear.setSelectedIndex(0);
		cboEndOnMonth.setSelectedIndex(0);
		cboEndOnDay.setSelectedIndex(0);
	}
