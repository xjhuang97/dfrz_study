...
public class StudentUpdateFrm extends JFrame{
	...
	public StudentUpdateFrm(StudentBean bean) {
		this.bean = bean;
		initUI();
		initFrame();
		configureCompents();
	...
	private void configureTxtClassRoomName() {
		...
		txtClassRoomName.setEditable(false);
	}
	...
	private void refresh() {
		txtID.setText(bean.getStudentID()+"");
		txtLoginName.setText(bean.getLoginName());
		txtGradeName.setText(bean.getGradeName());
		txtClassRoomName.setText(bean.getClassRoomName());
